...
if option == 'Analysis':
    ...
    unsafe_allow_html=True,)
        ...
        with col1:
          st.markdown(
    f"<h1 style='color:#ff6666; font-size: 20px;'>Top  Importers based on the quantity of rice exported</h1>",
    unsafe_allow_html=True,)
    ...
    unsafe_allow_html=True,)
        ...
        with col1:
          st.markdown(
    f"<h1 style='color:#ff6666; font-size: 20px;'>Top  Importers based on the IMPORT VALUE FOB of rice exported</h1>",
    unsafe_allow_html=True,)
  ...
  with tab3:
    ...
    if button2:
        ...
        st.markdown(f"<h1 style='color:#ff6666; font-size: 24px;'>Top 10 Importers</h1>",
                unsafe_allow_html=True,) 
        grouped_data = filtered_data.groupby(["IMPORTER NAME","YEAR"])["IMPORT VALUE FOB"].sum().reset_index().sort_values(by="IMPORT VALUE FOB",ascending=False).head(10)
        st.dataframe(grouped_data)
        ...
        st.markdown(f"<h1 style='color:#ff6666; font-size: 24px;'>Top 10 Exporters</h1>",
                unsafe_allow_html=True,) 
        grouped_data = filtered_data.groupby(["EXPORTER NAME","YEAR"])["IMPORT VALUE FOB"].sum().reset_index().sort_values(by="IMPORT VALUE FOB",ascending=False).head(10)
        st.dataframe(grouped_data)
        fig = px.bar(data_frame= grouped_data ,
                      x='EXPORTER NAME',
                     y='IMPORT VALUE FOB',
